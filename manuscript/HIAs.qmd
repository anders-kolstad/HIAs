...
# The data contains some multisurfaces as well
...
# I first tried to import and crop the mire data using stars, but that failed (see pre 21 feb 2023)
...
```{r test-function}
#| eval: false
test_sd <- wgt_mean(x = nature3$i_ADSV[!is.na(nature3$i_ADSV)], 
                 weights = drop_units(nature3$area[!is.na(nature3$i_ADSV)]), 
                 sigma.x = national_sd_ADSV,
                 stat = "sd")
test_mean <- wgt_mean(x = nature3$i_ADSV[!is.na(nature3$i_ADSV)], 
                 weights = drop_units(nature3$area[!is.na(nature3$i_ADSV)]), 
                 sigma.x = national_sd_ADSV,
                 stat = "mean")

hist(rnorm(1000, test_mean, test_sd))
...
```{r}
# Next I need to 
#  - copy the weighted means and SDs over to the ecosystem delineation map polygons 
...
#| ecosystem condition indicators along a Human Impact Gradient" 
