...
editor: 
  markdown: 
    wrap: sentence
# set eval to true to actually run the analysis
execute:
  echo: false
  include: false
  eval: false
...
<!--# The document is set to eval=F in the yaml, and some code chunks are set manually to eval=T. When working on the manuscript, you cannot run all with  Ctr+Alt+R as this ignores the YAML. This will read inn data cashed locally. To also evaluate the more time consuming chunks, change the YAML to eval: true and run all (or render to html):  -->
...
# Caululate the area of mire inside the coverage maps
# -- Nordre Follo
...
infra <- readRDS(paste0(path, "infrastructureIndex_discrete_vectorized.rds"))
...
infraMuni3_summary <- infraMuni3_tbl |>
  ...
  summarise(meanHIA = 
              round(
                weighted.mean(
                  infrastructureIndex, total_area_HIAs_km2), 2))

# Make a plot to check that it has worked 
infraMuni3_tbl |>
  ggplot()+
  geom_bar(aes(x = infrastructureIndex, 
  ...
  theme_minimal_hgrid()+
  labs(x = "Human Impact Area (HIA) class",
       y = "Area (km<sup>2</sup>)")+
  theme(axis.title.x = element_textbox_simple(
      width = NULL,
...
#cowplot::plot_grid(tmap_grob(muniPlot),
#                   tmap_grob(infraMuniMap),
#                   ncol=1,)
