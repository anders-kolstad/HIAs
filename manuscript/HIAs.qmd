...
<!--# The document is set to eval=F in the yaml, and some code chunks are set manually to eval=T. When working on the manuscript, you can run all (Ctr+Alt+R). This will read inn data cashed locally. To also evaluate the more time consuming chunks, change the YAML to eval: true and run all (or render to html):  -->
