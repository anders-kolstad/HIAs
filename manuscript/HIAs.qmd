...
combineALl <- rbind(
  ...
  rowwise() |>
  mutate(
    i_sample = list(sample(rnorm(10, mean, sd)))) |>
  unnest(cols=i_sample) |>
  summarise(mean = mean(i_sample),
            lower = quantile(i_sample, probs = c(0.025)),
            upper = quantile(i_sample, probs = c(0.975)))

# add area weights to the above and make forest plot

  
#combineALl
#tail(combineALl)
#combineALl$i_sample[2]
```
