...
# 117k polygons (2022)
# sometimes this function struggles with Norwegian letters.
...
# I now want to take the variable and normalise it before I can then combine them
# despite them being on different scales
# I will first normalise by converting into %.
# Remeber the ordinal categories represnts frequency ranges
...
# I will use th lower bound for each bin.
...
naturetypes <- naturetypes %>%
  mutate(value = case_when(
    # selecting the variable that follow the same 4 step scale
    variable %in% c("7TK", "7SE", "7FA") ~ 
      case_match(value,
             0 ~ 0,
             1 ~ mean(c(0, 1/16))*100,
             2 ~ 1/16*100,
             3 ~ 50), # note that it is not possible to get a value of 1
...
naturetypes_wide %>%
  ...
  geom_bar(stat="identity",
           ...
           colour = "black")+
  theme_bw(base_size = 12)+
  labs(x = "7TK or PRTK score",
       y = "Number of localities")
)

# The NA's represnets localities where just one of the two variables 
...
# Sum is problematic is not all locations ahve two values to sum together.
...
# And if we have 50% vehicle damaeg and 50% hiking damage, that is no doubt
...
naturetypes <- naturetypes |>
  ...
  # >
  #pivot_longer(cols = c(ADSV, "7FA", "7GR-GI"),
  #             names_to = "variable",
  #             values_drop_na = T)
```
...
upper2 <- 1
lower2 <- 5
threshold2 <- 2.5 # = observable effect. Value 3 indicates a shit to a new type (grunntype)
...
naturetypes$i_ADSV <- eaTools::ea_normalise(data = naturetypes,
                      vector = "ADSV",
                      upper_reference_level = lower,
                      lower_reference_level = upper,
                      break_point = threshold,
...
naturetypes$i_alien <- eaTools::ea_normalise(data = naturetypes,
                      vector = "7FA",
                      upper_reference_level = lower,
                      lower_reference_level = upper,
                      break_point = threshold,
...
naturetypes$i_ditch <- eaTools::ea_normalise(data = naturetypes,
                      vector = "7GR-GI",
                      upper_reference_level = lower2,
                      lower_reference_level = upper2,
                      break_point = threshold2,
...
#corrCheck |>
#  as_tibble() |>
#  mutate(i_ADSV_fct = floor(round(i_ADSV*10,2))/10,
#         i_alien_fct = floor(round(i_alien*10,2))/10) |>
#  count(i_ADSV_fct, infrastructureIndex) |>
#  View()
...
corrCheck |>
  ...
  pivot_longer(cols = c(i_ADSV_fct, i_alien_fct, i_ditch_fct),
...
# The year effect is constant, so ignoring that
corrCheck |>
  mutate(i_ADSV_fct = floor(round(i_ADSV*10,2))/10,
         i_alien_fct = floor(round(i_alien*10,2))/10,
         i_ditch_fct = floor(round(i_ditch*10,2))/10) |>
...
# Using a colour gradient emphasises the first colour (dark green).
# Lets try discrete colours, and merge som classes to simplify
corrCheck |>
  pivot_longer(cols = c(i_ADSV, i_alien, i_ditch),
               values_to = "indicatorValue",
               names_to = "indicator",
               values_drop_na = T) |>
  mutate(condition = case_when(
    indicatorValue <0.6 ~"Poor",
    indicatorValue <0.8 ~"Intermediate",
    indicatorValue <0.91 ~"Good",
    .default = "Perfect"
  ),
...
# The figure changes a lot depending on which value gets the darker colour
corrCheck |>
  pivot_longer(cols = c(i_ADSV, i_alien, i_ditch),
               values_to = "indicatorValue",
               names_to = "indicator",
...
# If I look at this on a log scale, it also changes and becomes less convincing for ADSV and alien
corrCheck |>
  pivot_longer(cols = c(i_ADSV, i_alien, i_ditch),
               values_to = "indicatorValue",
               names_to = "indicator",
...
# This is not correct I think as it evens out differences that are clearly there.
# i_ditch is shown less of a pattern for example
```

```{r}
#| eval: false

# HIAs 1 and 2 are quite similar for i_alien and i_ADSV, so perhaps I can combine them
corrCheck |>
  pivot_longer(cols = c(i_ADSV, i_alien),
               values_to = "indicatorValue",
               names_to = "indicator") |>
  ...
  condition = fct_reorder(condition, indicatorValue),
  HIA = case_when(
    infrastructureIndex %in% c(1,2) ~ "1 & 2",
    .default = factor(infrastructureIndex)
  )
  ) |>
  ggplot(aes(
    x = HIA, 
