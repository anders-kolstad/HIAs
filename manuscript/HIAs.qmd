...
# Taking he sum of 7SE and 7TK (incl the PR.. variabels)
naturetypes_wide <- naturetypes_wide |>
  rowwise() |>
...
naturetypes <- naturetypes |>
  left_join(naturetypes_wide |> select(id, var_7FA, ADSV), by = "id")
```

```{r rescale}
#| eval: true

# Now I rescale the now continous variables using reference and threshold values
...
# If I look at this on a log scale, it also changes and becomes less convincing
corrCheck |>
  pivot_longer(cols = c(i_ADSV, i_alien),
               values_to = "indicatorValue",
               names_to = "indicator") |>
