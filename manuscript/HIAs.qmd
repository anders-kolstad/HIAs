...
In this survey, selected nature types are delineated on a map (over 140 000 polygons at the end of 2023), and each locality is scored on a range of variables relevant for describing the state and quality of nature (REF INSTRUKS).
...
However, we do assume that alien plants are slightly less common on bogs relative to fens, and therefore that this variable will biased since that variable for only recorded in fens.

From the survey data set we identified six relevnt variables (variable 1-6 in @tbl-variables) which we attribute to three different ecosystem condition characteristics that describe the typical behavior of open mire ecosystems in the reference condition @fig-workflow.

Variables 1 -5 were were originally recorded along binned frequency ranges.
...
Variables 2-5 describe very related aspects, and are attributed to the same ecosystem condition characteristic (vegetation intactness) and so they where combined into a single indicator called antropogenic disturbance to soil and vegetation, or ADSV for short.
...
To turn the variables into indicators we scaled them using three normative reference values each: an upper (best possible condition), a lower (worst possible condition), and a threshold value that defines the breaking point between good and poor condition.
...
The threshold for *good ecosystem condition* was defined as 10%, which was then mapped to the value 0.6 on the rescaled indicators [@fig-scaling].
The variable 7GR-GI was rescaled into the indicator named *trenchin* by using the lower and upper reference values 1 and 5, respectively, and a threshold value of 2.5.
A variable value of 1 indicates an intact mire, and a value of 5 indicates a mire transitioning away from a wetland.
A value of 2 indicates observable change within the range expected for the same mapping unit, and a value of 3 indicates a mire transitioning into a neighboring (ecologically speaking) mapping unit.
See @fig-workflow for a schematic workflow for the indicator development.

*Alien species* was assigned to ECT class B1 - Compositional state characteristics and *ADSV* and *Trenching* were both assigned to ECT class A1 - Physical state characteristics [@czucz_common_2021].
...
There has not been a national assessment of the ecosystem condition of mires in Norway, but the authors recent contributed on a report which presented several new indicators that can be used in future assessments for this ecosystem [@nybo_indikatorer_2023; see also @kolstad2023]).
...
| 7   | Infrastructure Index | Infrastructure Index                             | Unitless linear scale from 0 to 13.2                                                    | Unitelss index ranging from from 0 to 13.2                                                                                                                        | @erikstad_index_2023               |
...
THe municipalities differ in several aspects, such as the amount of mire area, the total area surveyed, and the prevalence of infrastructure (@tbl-munis, @fig-studyLocation2, Figure S1).
...
Because the nature type survey data is spatially biased, we cannot simply assume that they are area representative.
In an attempt to overcome this issue we divided the area of Norway into four non-overlapping Homogeneous Impact Areas (HIAs) based on an infrastructure index [@tbl-variables, @erikstad_index_2023].
This index measures the frequency of different infrastructure types inside 500 m radius circles around each 10×10 m pixel.
We categorised this continuous variable into four classes (0–3) using the following value ranges: 0 = \<1; 1 ≤ 1 ≥ 6; 6 ≤ 2 ≥12; 3 ≥ 12.
We plotted this categorical map over an area in Norway that we are familiar with and confirmed a sensible seperation of areas from areas with no roads of major infrastructure (HIA = 0) to urban areas (HIA = 3).
We then aggregated the data to 1×1 km pixels to ease computations.
...
For each HIA and municipality combination we then created a probability distribution for the indicator values using Bayesian updating of a uniform prior between 0 and 1.
...
#| out.width: '100%'
#| fig.cap: |
#| - "Simplified schematic showing the workflow (Figures a-E) for horizontal aggregation of a spatially biased ecosystem condition indicator. Figure A shows a spatially explicit indicator. The colors indicator different indicator values (highlighted in the inset). The outline is the ecosystem accounting area (EEA). Figure B shows the distributio of four homogeneous impact areas (HIAs) inside the EEA. The indicator values in Figure A are used to update a uniform prior to produce a posterior probability distributio for the mean indicator value for each HIA (Figure C). The bayesian updating is informed by the standard deviation for the indicator in the much bigger national data set. Unlike an arithmetic approach where the variance in the mean indicator value per HIA is unreligable for very small sample sizes, the posterior distribution gives a religable measure for the indicator uncertainty even with a singe observation. The distributions in Figure C are assumed normally distributed. In Figure D the mean and SD from the posterior distributions are transferred to individual polygons in an ecosystem delineation map, for each HIA seperately. For each ecosystem occurence (i.e. ecosystem assets), we draw one random value for each square meter from a normal distribution with the mean and SD that is associated with that polygon. In Figure E we randomly sample 1000 values from the entire vector of sample in the previous step, and we get a area weighted probability distribution for the mean indicator value for the EEA."
