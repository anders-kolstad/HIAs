...
# There is very little data from HIA 3, so I combine it with HIA 2
corrCheck |>
  mutate(i_ADSV_fct = floor(round(i_ADSV*10,2))/10,
         i_alien_fct = floor(round(i_alien*10,2))/10,
         infrastructureIndex = case_when(
           infrastructureIndex == 3 ~ 2,
           .default = infrastructureIndex
         )) |>
  pivot_longer(cols = c(i_ADSV_fct, i_alien_fct),
               values_to = "indicatorValue",
               names_to = "indicator") |>
...
# All the 1's might make it hard to see the pattern.
# Try removing the 1's (the areas in best condition)
...
corrCheck |>
  ...
  pivot_longer(cols = c(i_ADSV_fct, i_alien_fct),
               values_to = "indicatorValue",
               names_to = "indicator") |>
  mutate(indicatorValue = na_if(indicatorValue, 1)) |>
  filter(!is.na(indicatorValue)) |>
  ggplot(aes(
    x = factor(infrastructureIndex), 
    fill = factor(round(indicatorValue,2))))+
