...
These municipalities are spread over `r round(max(latlong$Y)-min(latlong$Y), 1)` degrees latitude, and also differ in the amount of mire area, the total area surveyed, and the prevalence of infrastructure (@tbl-variables).
