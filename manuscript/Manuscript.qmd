...
(forest_plot <- stats_tbl |>
  ...
  labs(fill = "HIA",
       col = "HIA") +
  facet_grid(Municipality ~indicator)
)
...
# the polygons in the EDM (i.e. no influence fro the national dataset).
...
combineAll <- rbind(
  ...
  slice_sample(n = 10000, weight_by = area, replace = T) |>
  ungroup() |>
  select(Municipality,
         indicator,
         PDs,
...
#| eval: true
#| cache: true
# Now the same as above, but where the height of the blue bars reflect the EDM area and not the count
bin_width <- 0.1
