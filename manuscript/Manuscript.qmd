...
This recommendation for spatial resolution also means that when a condition metric is sampled coarsely, for example in an opportunistic and spatially biased field campaign, then it is very likely that you will not have a enough data points to to religably estimate an indicator value for each EA.
...
There are at least 2 ways to achieve this same complete areal coverage, that is relatively unbiased, from originally point-based indicator values:
...
b.  Simple projection of some best estimate, for example a central tendency from a stratified selection of area representative data
...
In this study we focused on open mire ecosystems in Norway where the thematic bias is less severe.
...
We then drew 1000 random values from this vector of possible indicator values and created a probability distribution for the indicators for the entire municipalities (i.e. the ecosystem accounting areas).
