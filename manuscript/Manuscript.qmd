...
Ecosystem condition accounting is the process of compiling relevant data on the status, trends and qualities of ecosystems (i.e. nature) and communicating this in a structured format [@comte2022; @edens2022].
...
A statistical standard for ecosystem accounting, including ecosystem condition accounting, was developed by the UN and adopted by the UN Statistical Commission in 2021 and is called SEEA EA (@unitednations2024; System of Environmental-Economic Accounting - Ecosystem Accounting).
The standard, or framework, is a set of rules, principles and best practices (see also @unitednations2022) for compiling ecosystem accounts, mainly aimed at national accounts.
At its core is the idea that data on ecosystem condition is summarised in the form of metrics (indicators and variables) that can be presented in gereralised tables that describe the condition of a larger ecosystem accounting area (EAA).

The SEEA EA framework recommends that ecosystem condition metrics give an unbiased representation of the condition for a given area [table 1 in @czucz_selection_2021; see also @unitednations2024 §2.87], and that condition metrics are recorded (or modeled) uniquely for all contiguous spaces (patches) of a specific ecosystem type (ecosystem asset) so that the condition in the EAA can be inferred from an area weighted mean based on the relative extents of the ecosystem assets [@unitednations2024 §5.54].
...
Note that the same issues of representativity also relate to thematic biases.
...
However, in this paper we chose to focus on spatial (geographic and environmental) bias.

Wall-to-wall raw data, such as satellite imagery, is spatially representative though the property of being spatially exhaustive, although of coarse the grain size also plays a role.
However, many ecosystem characteristics cannot be asserted from remote sensing.
Examples include things like species presences or abundances, community composition, soils, insects, chemical state, and more cryptic or smaller-scale physical state variables like deadwood or terrain or vegetation damage.
To complete an ecosystem condition account, field recorded data is therefore still essential.
However, due to limited sampling efforts, field campaigns often results on data scarcity with not enough data points to reliably estimate an indicator or variable value for each ecosystem asset.
The ecosystem assets with no data can either be ignored or populated with data using either some type of model or spatial generalisation (e.g. look-up tables).
Another common solution is to bypass the ecosystem assets level in the accounting directly estimate variable or indicator values for regions or the entire EAA by taking a central tendency (e.g. the mean) of the data.
...
Field recorded data that do not cover the entire EAA with a known inclusion probability can then either be limited in geographical extent, biased in environmental space, or both.
Here we focus in data with a bias in the environmental space.
The recommendation for representativity in the condition metrics means that spatially and environmentally biased data are ill suited, especially if sampling intensity varies systematically along gradients of anthropogenic pressures and hence ecosystem condition.
This bias should be controlled for in some way if the data is to be used in an ecosystem condition account.
@unitednations2022 list six relevant modeling techniques for ecosystem condition metrics that can also be sued for projecting metric values to ecosystem assets without field data, and simultaneously control for sampling biases.
...
Depending on the data that goes into models of type 2-6, they can make very precise and good indicators, especially when the EAAs are large (e.g. regions or nations).
However, in ecosystem accounting, data from outside the EAA should generally not be allowed to influence the results.
Consider a municipality creating an ecosystem condition account.
They would perhaps appreciate if the account reflected their local land use policies and nature management in general, and that it was unaffected by the actions of neighboring municipalities, or municipalities with similar ecological and climatic condition.
When EAAs are small, like the scale of a municipality, data usually becomes scarce and these types of models become associated with large uncertainties.

Look-up tables are the most simple modelling technique in the list above.
Typically with this technique, indicator or variable values are assigned to categorical classes or strata (e.g. ecosystem type within a geographical space), often with no uncertainty measure associated with it.
The value is then extrapolated to all areas of that class.
The level of extrapolation is subject to the size of the classes, with fine-grained typologies leading to less extrapolation.
How values are assigned to strata varies a lot, including expert opinion and numeric aggregation of data (e.g. the mean of all the data that fall in the same strata).
If the strata are defined according to relevant gradients in pressures or environmental conditions that might affect the metric, then this method may act as post-stratification to limit the degree of extrapolation and propagation of the initial sampling biases.
The most attractive properties of the look-up tables is perhaps the simplicity and the fact that it does not depend on external data (can use local data only).
The major drawbacks are the data requirements, as local data usually implies scarce data, and the lack of methods for estimating and presenting uncertainties around the estimates.

Projecting (inter- or extrapolating) non-continuous spatial data (e.g. points) over to a surface is a type of spatial generalisation.
This step require attention to sampling bias to ensure the projected values are as accurate as possible, as we have explained above.
The spatial grain of the data (and thus also the information) is essentially still the same.
A central part of ecosystem accounting is therefore the upscaling of information, essentially synthesizing detailed data to inform about the status of a larger area.
Other ways to think about this step is as reducing the grain or as increasing the support [@bierkens2002].
Upscaling is required to make the information available for decition makers or monitoring frameworks at the appropriate spatial scale [@king2024].
In ecosystem accounting this is typicaly achived by aggregation of data, i.e. combining several data points into one, for example by taking the arithmetic mean.
Another methods include sampling [@brus1997] or resampling techniques [@jakobsson2021].
For simplicity we also refer to this as aggregation, as the end result is the same - a synthesis of information.

Spatial aggregation, for example from ecosystem assets to EAAs, allows for the quantification of spatial variance in the estimate.
In addition, there can be errors, e.g. measurement errors, associated with the raw data that could potentially be propagated thought to the upscaled estimate [e.g. @heris2021].
However, SEEA EA has no clear guidelines for estimating or disclosing uncertainties in accounts, whether it be spatial variance or other errors.
This is problematic and could reduce the credibility and finally the uptake of knowledge from ecosystem accounting [@bagstad2021, @venter2024].
...
<!--# Taking fine-grain data and making conclusion about largeer areas can be achieved by either aggregation or projection. More generally we can call this upscaling. Scale-changes or spatial scaling has many potential pit-falls, related to aggregation error (Rastetter), MAUP and the choice of basal spatial units, the ecological fallacy.  -->
...
The technique consist of using bayesian updating to get uncertainties surrounding the estimates, whilst being able to use very scarce data sets.
