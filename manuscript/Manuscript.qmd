...
wgt_mean <- function(
  ...
  # we can assume that the distrubution for the mean is gaussian, so we take a 
...
tempDF <- tibble(
  ...
  mutate(
    ...
    PD_15 = list(
      wgt_mean(
        x = unlist(population_sample)[1:15],
        weights = unlist(wgt)[1:15],
        shape1 = shape1,
        shape2 = shape2
      )
    ),
    ...
    sample_15 =
      list(unname(unlist(population_sample)[1:15])),
    # adding unweighted sample means
    mean_5 = mean(sample_5),
    mean_15 = mean(sample_15)
  )
...
tempDF_nest <- tempDF |>
  #select(-wgt) |>
  pivot_longer(cols = !starts_with("wgt"),
               names_to = "distribution",
               values_to = "population_sample") |>
  nest(data = population_sample)
tempDF |>
  ggplot()+
...
tempDF |>
  ggplot()+
    ...
    facet_wrap(vars(distribution),
      scales = "free")
# Very similar, also for the gaussian and uniform population samples
```

```{r testCont2}
size <- 1000
# estimate sub popualtion means and errors as posterior distributions

# sample size = 1

PDs <-tibble(
    normal_centered = wgt_mean(
      tempDF$pop_normal_centered[1], 
      tempDF$wgt[1],
      shape1 = pop_normal_centered_para$parameters["shape1"],
      shape2 = pop_normal_centered_para$parameters["shape2"])[1:size],

    pd_beta_05_3_1 = wgt_mean(tempDF$pop_beta_05_3[1], 
      tempDF$wgt[1],
      shape1 = pop_beta_05_3_para$parameters["shape1"],
      shape2 = pop_beta_05_3_para$parameters["shape2"])[1:size],
    pd_beta_05_3_5 = wgt_mean(tempDF$pop_beta_05_3[1:5], 
      tempDF$wgt[1:5],
      shape1 = pop_beta_05_3_para$parameters["shape1"],
      shape2 = pop_beta_05_3_para$parameters["shape2"])[1:size],
    pd_beta_05_3_20 = wgt_mean(tempDF$pop_beta_05_3[1:20], 
      tempDF$wgt[1:20],
      shape1 = pop_beta_05_3_para$parameters["shape1"],
      shape2 = pop_beta_05_3_para$parameters["shape2"])[1:size]
  )

    #pd_uniform_1 = wgt_mean(
    #  tempDF$pop_uniform[1], 
    #  tempDF$wgt[1],
    #  shape1 = pop_uniform_para$parameters["shape1"],
    #  shape2 = pop_uniform_para$parameters["shape2"])[1:100],

    pd_normal_centered_5 = wgt_mean(tempDF$pop_normal_centered[1:5], 
      tempDF$wgt[1:5],
      shape1 = pop_normal_centered_para$parameters["shape1"],
      shape2 = pop_normal_centered_para$parameters["shape2"])[1:size],
    pd_normal_centered_20 = wgt_mean(tempDF$pop_normal_centered[1:20], 
      tempDF$wgt[1:20],
      shape1 = pop_normal_centered_para$parameters["shape1"],
      shape2 = pop_normal_centered_para$parameters["shape2"])[1:size],
...
PDs |>
  pivot_longer(cols = everything(),
               names_to = "populations_dist",
               values_to = "PD") |>
  ggplot()+
  geom_histogram(aes(x = PD),
                 binwidth = 0.1)+
  facet_wrap(vars(populations_dist),
             scales = "free")
```

```{r}
test <- wgt_mean(tempDF$pop_beta_05_3[1], 
      tempDF$wgt[1],
      shape1 = pop_beta_05_3_para$parameters["shape1"],
      shape2 = pop_beta_05_3_para$parameters["shape2"])[1:100]

par(mfrow = c(1,2))
hist(test) 
hist(tempDF$pop_beta_05_3)
# complete nonsense
```
