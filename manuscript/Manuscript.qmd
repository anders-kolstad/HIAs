...
However, there is a surge of examples of more local accounts appearing, motivated at least partly by the need to match the scale of the information to the scale of decision-making processes.
...
In this paper we have demonstrated a generalisable workflow for making use of scarce and spatially biased field data to produce representative ecosystem condition indicators, including uncertainty estimates, at a local municipal scale (@fig-workflowExample).
These approaches can provide valuable tools for local ECAs where limited samples and low data availability still are the major limitation.
...
Ecosystem condition indicators should be spatially representative for the individual ecosystem assets in an ECA so that an area weighted average across ecosystem assets provides a representative estimate for the given ecosystem type.
Remotely sensed data are spatially representative if they are spatially exhaustive, and they can be used for indicators in ECAs at any scale.
However, many aspects of ecological condition cannot be remotely sensed.
At national or international scales, indicator developers may have some field-based monitoring data available to them which are sampled using a random or systematic design, and thus fulfill the criteria of representativeness.
However, for local scales, such surveys or monitoring programs are less common.
...
This includes data from certain citizen science initiatives, or as in our case, spatially clumped nature type mapping with unknown environmental biases.
Being able to include these kinds of data in local ECAs would greatly alleviate data shortages and provide essential field-based data on for example species composition or small scale disturbances.

Or solution to this problem was to identify a candidate spatial variable reflecting the direct negative drivers of the ecosystem characteristics, and thus the indicators themselves.
...
In the case for *Alien species*, stratifying the variable by the the prevalence of infrastructure probably made less sense and other HIA definitions could have been explored, for example taking into account latitude as there are gernerally more alien species in the south compared to the north in Norway.
The drawback that we see from using this stratification technique when the HIA variable is not a good predictor for variation in the indicator (as for *Alien species*), is that we end up with less data points and more stochacity in the indicator point estimate, than we otherwise would.
...
## Small sample sizes and small area statistics
...
Our HIA was an ordinal gradient from low to high prevalence of infrastructure, and
