---
title: Local ecosystem condition accounting – leveraging post-stratification and small domain estimation to incorporate scarce and spatially biased data
author:
...
# Ecosystem delineation map - unfortunately to publically available
...
path_naturetypes <- "P:/41001581_egenutvikling_anders_kolstad/data/Naturtyper_nin_0000_norge_25833_FILEGDB.gdb"

# municipality outline
path_muni <- here("data/Basisdata_0000_Norge_25833_Kommuner_FGDB.gdb")
...
# Set local to ensure Norwegian letters get imported correctly
...
# These are the variabels were after
...
# To crop the EDM, I need the three municipalities seprately.
nf <- muni3 |>
...
domain_data <- tibble(
    ...
    values = list(
      ...
      # indicator values are often pseudo-continous,
      # so ading a special case of nondistinct values
...
update_distribution <- function(domain_data, national_fit, prior_weight = 10) {
  ...
  # In those cases I will use tha national shape,
  # To check if the function produces new or reused the old paramteres, one can 
  # unhash this line:
...
# and reduced the CI.
# Exeption for  the single values, where the domain has less 1s 
# than the national data and thus increases the PD varaition with n.
...
# not be the case. One option is to get more spesific priors, for example not the
...
# this ralative measure of variance over to be centered on the domain mean, but 
...
# traditinal errors. 
# The distributions are asymetrical, which is natural for truncated data,
...
However, there is a surge of examples of more local accounts appearing, motivated at least partly by the need to match the scale of the information to the scale of decision-making processes (**REF**!).
...
The issue of representativenes extends beyond spatial biases to include thematic biases—such as the under-representation of key ecosystem components like insects or soil biota.
...
Ecosystem accounts should ideally reflect conditions only insde the accounting area and are arguable more useful when reflecting local management and land use policies, unaffected by the actions of neighboring or ecologically similar administrative units.
...
Values may derive from expert opinion or aggregated data (e.g. straum means).
...
After expotrapolating data, the spatial grain of the data (and thus also the information) is essentially still the same.
...
In addition, there can be errors, e.g. measurement errors, associated with the raw data that could potentially be propagated and reported alongside the upscaled estimate [e.g. @heris2021].
Currently, SEEA EA lacks guidelines for estimating or disclosing uncertainties in accounts, whether it be spatial variance or other errors.
...
In addition, calculating variances is even more data demainding when metrics are truncated, such as for indicators bound between zero and one.
In such cases, a small sample size can contain only or predominantly ones (or zeros), resulting in an artificially small variance which will then paradoxically increase as sample sizes increase due to the inclusion of more data points from the tail of the distribution.
...
In this study we present two techniques for creating local ecosystem condition indicators using scarce and spatially biased data.
...
Second, we predict indicator distributions and uncertainties using an empirical Bayes approach to small area estimation where we borrow strength from a larger national dataset, and we combine this with a requirement for making indicator point estimates using local data only.
We demonstrate these techniques with three indicators from the same spatially biased nature type mapping data set from Norwegian mires, comparing results across three municipalities in Norway to illustrate its potential for local applications.
...
Northern fens require larger size and higher calcareousness to be surveyed.
...
We define the reference condition as one where ecosystems are subject to little or no human influence, with a climate as in the period 1961-1990 and a native species pools similar as today [@jakobsson2021].
...
Three municipalities were selected test the indicators, varying in mire area, survey coverage, and infrastructure prevalence (@tbl-munis, @fig-studyLocation2, S3, S4).
...
We did the same for the local strata, although the estimation of the shape parameters requires a certain sample size and dispersion of data values to converge, making it impossible for some strata to describe the distribution of the samples.
Using Bayesian updating we combined prior information about the shape of the national-level data with the local strata data using a prior weight of 20, meaning the national data informed about the shape of the local indicator distribution with similar weight as 20 local data points.
...
Still, the prior weight was low enough that all point estimated (calculated independently from the predicted distributions) fell inside the 95% credible intervals.
The predicted distributions were transferred over to the ecosystem assets with the corresponding strata, and from there, values where sampled proportional to asset area to obtain a distribution for the EAA.

(@fig-workflowExample - C; Appendix C, line 134 <!--# update line number manually when re-rendering appendix C -->).
...
The indicator values were comparable across HISs in the three municipalities, although there were some differences, as for the *Trenching* indicator where in Gran, HIA-2 had worse condition than HIA-1 (@fig-forest).
...
Nordre Follo also had the highest (the best) indicator values for one of the indicators (ADSV); probably reflecting stochaticity due to low sample size .
...
In this paper we have demonstrated a generalisable workflow for making use of scarce and spatially biased field data to produce representative ecosystem condition indicators, with uncertainty estimates, at a local municipal scale (@fig-workflowExample).
These approaches can provide valuable tools for local ECAs where limited sample sizes and low data availability still are the major limitation.
We will focus on two novel approaches in turn, describing their virtues and uses: post-stratification for minimising adverse effects of spatial and environmental bias in the data, and empirical Bayes for calculating realistic indicator uncertainties even for small sample sizes.
The two approaches can also be used independently.

## Post-stratification approach using HIAs

<!--# Ecosystem condition indicators should be spatially representative for the individual ecosystem assets in an ECA so that an area weighted average across ecosystem assets provides a representative estimate for the given ecosystem type. Remotely sensed data are spatially representative if they are spatially exhaustive, and they can be used for indicators in ECAs at any scale. However, many aspects of ecological condition cannot be remotely sensed. At national or international scales, indicator developers may have some field-based monitoring data available to them which are sampled using a random or systematic design, and thus fulfill the criteria of representativeness. However, for local scales, such surveys or monitoring programs are less common.  A third category of datasets include opportunistically sampled data where both the relative sampling efforts and the local sample sizes can vary widely. This includes data from certain citizen science initiatives, or as in our case, spatially clumped nature type mapping with unknown environmental biases. Being able to include these kinds of data in local ECAs would greatly alleviate data shortages and provide essential field-based data on for example species composition or small scale disturbances.  -->

Local ECAs are usually not blessed with having a lot of area representative field data to base indicators on and are therefore forced to look at other data sources that might be less ideal, for example due to opportunistic sampling designs.
We provide a simple solution to make such data more compliant with SEEA EA. First we identifed a candidate spatial variable hypothesised to reflect the direct negative drivers of the ecosystem characteristics, and thus the indicators themselves.
With a categorised version of this variable we post-stratified the survey data and calculated indicator values for each strata separately, thus avoiding to extrapolate extensively beyond the geographic and environmental context where the samples where initially taken.
We refer to the strata as homogeneous impact areas, or HIAs.
...
Our HIA was an ordinal gradient from low to high prevalence of infrastructure.
We found a relationship of increasing infrastructure and decreasing indicator values ranging from weak, in the case of the *Alien species* indicator, to strong, for the indicator for *Trenching* in mires (the *ADSV* indicator falling somewhere in the middle) (@fig-validation).
...
In the case for *Alien species*, stratifying the variable by the the prevalence of infrastructure probably made less sense.
Other HIA definitions could have been explored, for example taking into account latitude as there are generally more alien species in the south compared to the north in Norway.
A drawback from using this stratification technique, especially when the HIA variable is not a good predictor for variation in the indicator, as for *Alien species*, is that we end up with less data points and more stochacity in the indicator point estimate, than we otherwise would.
However, we do not see this as a major problem.
Also, since our HIA areas are somewhat cohesive, forming a clumped or aggregated pattern, stratifying by it will on average lead to less geographic extrapolation.
...
We demonstrated the use of HIAs on local ecosystem condition indicators and we believe this is where the approach has the greatest benefits, such as the ability to work with small sample sizes.
In addition, local area managers may get useful information from seeing the indicators values uniquely for each HIA (@fig-forest) and in that way gain an understanding about where problems with ecosystem deterioration are greatest.
However, with larger EAAs and more data points, geostatistical methods such as krieging might be more suitable.
But these methods are also more complicated and thus less transparent to the end users, therefore at a compromise with the ideal for simplicity (@czucz_selection_2021).

When defining the HIAs, the main trade-off is between trying to keep the areas as small and homogeneous as possible, and at the same time ensure there are enough data points to estimate indicator values for each , or most, of the HIAs.
In our case, only two or three HIA levels had field data recorded in them, depending on the municipality, and the differences in indicator values between HIAs in each municipality was relatively small.
This could mean that broader HIA definitions could have been used in our case.
Alternatively, in case there are no data for a stratum, one might chose to impute values *post hoc*, instead of simply dropping the strata all together, as we did.

## Indicator uncertainties and small area statistics

<!--# This approach is useful especially for local assessments where we do not want to use data from outside the EAA to inform or influence our estimates for condition indicators inside the EAA. The method relies on stratifying the field data by Homogeneous Impact Areas (HIAs). Our HIA was an ordinal gradient from low to high prevalence of infrastructure, and -->

Local ECAs that use field data are often faced with a problem of low sample sizes.
This affect the religablility of both the indicator point estimate and the variance.
For the point estimate we do not offer a direct solution, and simply use the area weighted mean of the data that are available for any give strata.
This approach ensures that local ECAs are populated with information on local ecosystem characteristics, untainted by neighbouring areas where environmental policies might differ.
...
We hold this to be a fact and are therefore critical to the lack of direct guidelines for estimation and disclosure of uncertainties in the SEEA EA statistical standard.
The variance (or standard deviation) is perhaps most commonly used as a measure of uncertainty around indicator estimates, usually calculated from the variation between spatially distant sampling points.
However, the variance is not reliable for low sample sizes, nor is it sensible for highly skewed or truncated data.
If a numerical population has a large variance, it is still possible to sample, by chance alone, two or three numbers that are very close to each other, and this sample would then display a low variance which is not reflective of the true variance.
This problem get exaggerated when data is truncated, as indicators are (they are bound between zero and one), because one is more likely to sample a series of identical one's or zero's which would yield a variance of zero.
Knowing this, one approach is to set a sample size limit, and refrain from estimating indicator values (or at least the uncertainties) unless there are more than a given number of data points.
In our test municipality Nordre Follo, we had only 1-3 data points for each HIA strata, yet we were able to use these data regardless.
Our emipirical Bayes approach would in this case borrow strenght from a larger national dataset to predict the most probable distribution of indicator values, whilst keeping the point estimate seprerate from this process.
...
Our solution does not however, fix the inherent stochastic that follow from having a point estimate based on few data points, but it puts this estimate into the light of a realistic distribution of possible indicator values, thus making it possibel to extract actionable information from data that would otherwise had been discarded.

As we would not estimate indicator values without uncertainty estimates, then our approach make it possible to estimate indicator values for a much greater area than if we were forced to set a minimum sample size threshold.
With increasing sample sizes, local data get more infuence on predicting the indicator distributions.
The solution is therefore generic and can be used on large and small datasets alike, or in cases where some HIAs are well samples and some are not.
...
<!--# The UN standard for ecosystem accounting has been tailed towards national accounts [@unitednations2021]. Similarly, reporting of Ecosystem Accounts to Eurostat, expected to become mandatory for all EU and EEA countries from 2026, is unlikely to require sub-national resolution. Assessments at national scales can make use of data sets, and hence indicators, that local assessments cannot. For example, national forest inventories or other national area representative field-based nature monitoring provide enough data points for reliable estimates of indicator values only at relatively large spatial scales. Local ecosystem accounts instead rely more on remote sensing or field measurements. Remote sensing holds great promise for delivering valuable information to ecosystem accounts across all scales, yet there are some ecosystem characteristics that cannot easily be sensed remotely. This includes for example species identities, and hence indicators on topics such as alien species or occurrence of threatened or certain keystone species. Fieldwork will therefore still be vital for collection of information for ecosystem accounts also in the future. -->

<!--# To increase efficacy of fieldwork at local scales - and hence, reduce costs - our results demonstrate that municipalities can stratify their field sampling according to the HIAs. For example, Gran and Nord-Aurdal have not recorded any field data from their most urban areas (HIA 3) and could make these areas a priority in upcoming field campaigns in order to get more complete spatial indicator coverage. -->

<!--# We used a national data set of nature type surveys that follow a shared protocol [@norwegian_environmental_agency_protocol]. There can be other data sets, that may not be compiled nationally, but which could be relevant to local ECAs. In some cases, a local ECA is mostly interested in describing and monitoring the status within its own EAA, but great benefits may be had if accounts can also be compared between EAA, say between municipalities. This kind of information can give grounds for a more regional assessment, for example looking at the effects of policies on ecosystem condition trajectories. To accomplish this level of synthesis it helps to be able to use indicators that are common across EAAs. This all depends on the right variables being collected in a similar way, for example across different projects or monitoring frameworks. In Norway, ecosystem impact assessments (EIA) are frequently being produced in conjunction with planned development projects. This data could be made very valuable for local ecosystem condition accounts, but currently there are several things stopping this. Firstly, there is no standardised reporting for EIA in Norway, and no mandatory publication pathway, and therefore no way to obtain a unified data set of variables recorded across different EIAs. If data is made publicly available in a national data base, they could be used to design indicators that are informative at multiple scales, also larger than the project level scale at which they were recorded. Second, the data collection is mainly on the occurrence of specific nature type or species of interest, and true condition parameters are not part of the information required by the authorities. In contrast, other nations have implemented EIA or EIA equivalent systems that have much greater synergistic properties towards ecosystem accounting, such as the Biodiversity Net Gain program in England [@departmentforenvironmentfoodruralaffairs]. This program enforces developers to record and compile data on ecosystem extent and condition at very fine resolution in order to document their effect on nature and ensure a positive net gain from their activities. Relevant ecosystem condition variables are recorded for each delineated ecosystem areas, providing an explicit link between ecosystem type and condition. -->
...
The exception was for variable *Trenching* in Nordre Follo, where the indicator value was 0.24; well below the threshold for good ecosystem condition (0.6).
This estimate is based on just 4 data points (i.e. mires).
The predicted indicator distribution is higher, meaning that if Nordre Follo is similar to the rest of Norway, then increasing the sampling effort should elevate the indicator value.
However, Nordre Follo migh be unique, and dissimilar, to the rest of Norway, and for that reason, the point estimate is independent of data from outside the municipality and reflecting only the local condition.
...
We believe this indicator estimate is trustworthy, considering the wide, right skewed credible intervals, and that it could reflect the true prevalence of trenching in Nordre Follo.
...
<!--# we have demonstrated a new GIS based workflow for constructing ecosystem condition indicators from spatially biased variables using a method of stratified aggregation by an ordinal gradient of Homogeneous Impact Areas (HIA). In addition, we show how Bayesian updating can help produce uncertainty estimates for condition indicators that suffer from low sampling sizes where traditional variances would be unreliable. Our workflow can also help guide local field efforts to in the future get better and less resource demanding local ecosystem condition accounts. -->
