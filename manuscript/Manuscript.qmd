...
Ecosystem assets (EAs) are defined as "ecological entities" (meaning areas) about which information is sought and about which statistics are compiled [@unitednations2021].
...
The recommendation for spatial resolution also means that when a condition metric is sampled coarsely, for example in an opportunistic and spatially biased field campaign, then it is very likely that you will not have a enough data points to reliably estimate an indicator value for each EA.
...
Depending on the data that goes into these models, they can make very precice and good indicators (REF).
...
#| municipalities. The color gradient reflects the value of the x-axis. 
...
This approach is useful especially for local assessments were we do not want to use data from outside the EAA to inform or influence our estimates for condition indicators inside the EAA.
...
Remote sensing holds great promise for delivering valuable information to ecosystem accounts across all scales, yet there are some ecosystem characteristics that can not easily be sensed remotely.
...
We used a national data set of nature type surveys that follow a shared protocol (REF).
There can be other data sets, that may not be compiled nationally, but which could be relevant to XXX.
...
The exception was for variable *Trenching* in Nordre Follo, where the indicator value was 0.25; well below the threshold for good ecosystem condition (0.6) (REF Jakobsson 2020, 2021).
