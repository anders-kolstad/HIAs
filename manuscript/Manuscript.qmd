...
# I will use a beat distribution, since the indicator is bound by 0 and 1.
# I will estimate the two shape parameters using ML.
...
# It would be ossibel to also get uniqe SDs for each HIA class.
```
...
# This function is modified (quite a lot) from Bolstad::normdp
# It updates a flat uniform prior based on a sample of values (indicator values)
# and weight (polygon area) and returns a distribution for the weighted mean
# that has a beta distribution. The shape parameters are estimated from the full sample of indicator values.
# The updated mean will always be the sample mean, but the uncertanty is
# informed also by the national dataset (i.e. we borrow strenght, as a type of
# small area estimation)


#wgt_mean <- function(
#    x, # indicator values (the sample)
#    weights,  # polygon area
#    shape1, # shape parameter for the beta distribution
#    shape2, # second shape parameter for the beta distribution
#    # sigma.x = NULL, 
#    mu = seq(0, 1, length.out=1000), # discrete grid for estimation of probabilities
#    mu.prior = rep(1/length(mu), times=length(mu)),  # discrete uniform prior
#    stat = "pd", # output variable
#    ...) {
#  
#  # The mean will always be the area weighted mean of the sample,
#  # with no influence from priors
#  mx <- weighted.mean(x, weights)
#  
#  # Normalising the prior if needed
...
#  }
#  
#  # get the lenght of the estimation grid
#  #n.mu <- length(mu)
#  
#  # sample size
#  #nx <- length(x)
#  
#  # beta variance
#  snx <- (shape1*shape2)/((shape1+shape2)^2)*((shape1+shape2+1))
#  
#  # the weights are not used in the liklelihood function, just for the weighted mean 
#  likelihood <- exp(-0.5 * (mx - mu)^2 / snx)
#  posterior <- likelihood * mu.prior / sum(likelihood * mu.prior)
#  
#  # the posterior mean
#  mx2 <- sum(mu * posterior)
#  
#  
...
wgt_mean <- function(
    ...
    shape1, # shape parameter for the beta distribution
    shape2, # second shape parameter for the beta distribution
    mu = seq(0, 1, length.out=10000), # discrete grid for estimation of probabilities
    mu.prior = rep(1/length(mu), times=length(mu)),  # discrete uniform prior
    stat = "pd", # output variable
    ...) {
  ...
  # Calculate likelihood using beta distribution
  # For each mu in our grid, we treat it as the mean of a beta distribution
  # and calculate the product of likelihoods for all observations
  likelihood <- sapply(mu, function(m) {
    # Convert mean to alpha and beta parameters
    # For beta distribution: mean = shape1/(shape1+shape2)
    # We'll assume shape1+shape2 is constant (you might want to parameterize this differently)
    total_shape <- shape1 + shape2
    alpha <- m * total_shape
    beta <- total_shape - alpha
    
    # Calculate weighted likelihood (weights as importance weights)
    sum(weights * dbeta(x, alpha, beta, log = TRUE))
  })
  
  # Convert from log-likelihood and normalize
  likelihood <- exp(likelihood - max(likelihood)) # for numerical stability
  ...
  sample <- sample(mu, size = 1000, prob = posterior, replace = TRUE)
...
```{r fig-simSample}
tempDF |>
  unnest(simulated_sample) |>
  ggplot()+
    geom_histogram(aes(x = simulated_sample),
      binwidth = 0.1)+
    facet_wrap(vars(distribution),
      scales = "free")
# Very similar to the original, also for the gaussian and uniform population samples
```
...
sampleDF <- tempDF |>
  ...
  select(sample_size,
         sample,
         distribution) |>
  mutate(sample_size = str_remove(sample_size, "sample_")) |>
  unnest(sample)

wgtMeanDF <- tempDF |>
  pivot_longer(cols = starts_with("wgt_mean_"),
               names_to = "sample_size",
               values_to = "wgt_mean") |>
  select(sample_size,
         wgt_mean,
         distribution) |>
  mutate(sample_size = str_remove(sample_size, "wgt_mean_")) 

  
# plot posterior distribution
tempDF |>
  unnest(starts_with("PD")) |>
  pivot_longer(cols = starts_with("PD"),
               names_to = "sample_size",
               values_to = "PD") |>
  mutate(sample_size = str_remove(sample_size, "PD_")) |>
  bind_rows(sampleDF) |>
  bind_rows(wgtMeanDF) |>
  ggplot()+
  #geom_density(
  ...
  facet_grid(cols = vars(distribution),
             rows = vars(as.numeric(sample_size)),
             scales = "free")
#ggsave("../images/posteriorCheck.png")

# this seem reasonable, altough the pd is shifted slightly for the truncated normal (normal high)
# Not sure why by the main thing is that the errors are behaving as expected. 
# The means are in any case gives by the samples only, and are not affected by the PD. 
```
...
Ecosystem condition accounting is the structured compiling and communication of data on the status, trends and qualities of ecosystems [@comte2022; @edens2022].
...
At its core is the idea that data on ecosystem condition can be summarised in the form of metrics (indicators and variables) presented in gereralised tables that describe the condition of a larger ecosystem accounting area (EAA).
...
Ecosystem condition assessments are often constrained by data availability, leading to pragmatic (and potentially unrepresentative) selections of variables and indicators.
...
The issue of representativeness extends beyond spatial biases to include thematic biases—such as the underrepresentation of key ecosystem components like insects or soil biota.
...
Wall-to-wall raw data, such as satellite imagery, is spatially representative though the property of being spatially exhaustive, although grain size also plays a role [cf. the ecological fallacy; @robinson2009].
...
These surveys, however, often suffer from sparse or uneven (biased) sampling, leaving gaps in ecosystem asset coverage.
Field data are especially problematic if sampling intensity correlates with anthopogenic pressures (and thus ecosystem condition) [@aubry2024].
...
Small area estimation is separate branch in statistics finding ways to overcome this issue by "borrowing strength" from related areas [@ghosh1994; @guldin2021].
Ecosystem accounts should ideally reflect local conditions and are arguable more useful when reflecting local management and land use policies, unaffected by the actions of neighboring or ecologically similar administrative units.
There might therefore be a limit to how much data one should "borrow" across ecosystem assets or EAAs.

Look-up tables, the simplest approach, assign the same value to categorical strata (e.g. ecosystem type within a region), often with no uncertainty estimate (although this is quite possible to include).
...
The level of unwanted extrapolation is subject to the size of the strata, with fine-grained typologies leading to less extrapolation.
...
A critical step in ecosystem accounting is upscaling—synthesising fine-grained data to characterise larger areas, effectively reducing grain size or increasing support [@bierkens2002] (**Kolstad *et al.* in prep**).
...
In this study we present a stratified aggregation technique to leverage spatially biased field data in ecosystem condition accounting.
Using Bayesian updating, we incorporate uncertainty estimates, whilst accommodating scarce data sets.
We demonstrate this technique with three indicators from the same spatially biased nature type mapping data set from Norwegian mires, comparing results across three municipalities in Norway to illustrate its potential for local applications
...
We used a dataset of mapped nature types in Norway [@norwegian_environmental_agency_naturtyper_2024], surveyed following a standardised protocol [@norwegian_environmental_agency_protocol].
...
Surveys are spatially biased, typically towards areas with high human impact or expected impact, reflecting their primary use in land-use decisions.
...
We posit that our selected variables (see below) are relative unaffacted by this bias, with a possible exception for alien plant occurences, which are probably less common on bogs, but the variable is recorded only in fens.
...
Normalisation followed a piecewise linear transformation with three reference levels [@czúcz; \@jakobsson2020]
...
For each HIA — municipality combination we created a probability distribution for the area weighted mean of the indicator values using Bayesian updating of a uniform prior between 0 and 1, informed by national-level standard deviation (SD) (@fig-workflowExample - C; Appendix C, line 134 <!--# update line number manually when re-rendering appendix C -->).
The mean and SD from the assumed normally distributed posterior distributions were carried over to each mire polygon in the ecosystem delineation map, for each HIA class separately.
Then for each polygon in a municipality we sampled values from a normal distribution with this same mean and SD with sample size weighted by polygon area, thus ensuring that larger polygons contributed more samples.
Final indicator probability distributions for each municiality were made up of 1000 random draws from this larger vector distribution.
HIAs lacking data were ignored excluded from municipal estimates.
