...
Field data are especially problematic if sampling intensity correlates with anthopogenic pressures (and thus ecosystem condition).
...
Depending on the data that goes into them, models of type 2-6 can make very accurate estimates, especially when the EAAs are large (e.g. regions or nations), but their reliability diminishes for smaller areas (e.g., municipalities) due to data scarcity and model uncertainty.
Ecosystem accounts should ideally reflect local conditions, and data from outside the EAA should generally not be allowed to influence the results.
The local account is more useful if it reflectes local managment and land use policies, and and if it is unaffected by the actions of neighboring administrative units, or units with similar ecological and climatic condition.
...
In this study we presnet a stratified aggregation technique to leverage spatially biased field data in ecosystem condition accounting.
