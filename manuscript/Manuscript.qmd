...
This problem does not exist for complete wall-to-wall data, like remotely sensed imagery, where each EA will have ebough data to estaimte its unique indicator value.
...
c.  Take a central tendency from stratified sections of the sample population and projection them to the EAs, adhering to the corresponding strata.
...
The variable 7TK was rescaled into the indicator named *Alien species*, 7GR-GI was rescaled into the indicator *Trenching*, and ADSV was rescaled and kept the name *ADSV* (Figure S1).
...
*Alien species* was attributed to Ecosystem Condition Typology (ECT) class B1 - Compositional state characteristics and *ADSV,* and *Trenching* was attributed to ECT class A1 - Physical state characteristics [@czucz_common_2021].
...
We then categorised this continuous variable into four classes (0–3) using ranges chosen to produce a sensible and relatively linear area classification when visualised along an urban-wilderness gradient, whilst at the same time delineating sufficient areas for each strata across Norway (Table S1).
...
#| The data is from a national vegetation type survey in Norway." 
...
# Histogram smoothing makes it look like there are more data points beyong the lower and upper percentiles than there really is. The 2.5% percentile sometimes looks like the 25% percentile. 
...
This approach is useful especially for local assessments were we do not want to use data from outside the EAA inform or influence our estimates for condition indicators inside the EAA.
...
For example national forest inventories or other national area representative field-based nature monitoring provide enough data points for a reliable estimates of indicator values only at relatively large spatial scales.
...
However, field work is labour intensive and expensive.
...
Nordre Follo has surveyed more of its total land area compared to the other two municipalities, but has less mire area to begin with (@tbl-munis) , and that is the reason for this low sample size.
We believe this indicator estimate is trustworthy, in light of the wide credible intervals, and that it could reflect the true prevalence of trenching in Nordre Follo.
...
For a rendered version of the source file, with all code and calculation visible, see Appendix C. The source files and the data are also located on GitHub (https://github.com/anders-kolstad/HIAs) and will be archived on Zenodo with a stable doi at time of submission.
