...
With a categoised version of this variable we post-stratified the survey data and calculated indicator values for each strata separately, thus avoiding to extrapolate extensively beyond the geographic and environmental context where the samples where initially taken.
We refer to the strata as homogeneous impact areas, or HIAs.
A visual check confirmed that our HIAs were indeed associated with indicator values in an expected way.
A similar concept callad homogeneous ecosystem areas was introduced by @vallecillo_eu-wide_2022 and refer to parts of an ecosystem type that can be allowed to share common reference levels.
HIAs on the other hand, refer to areas that can be allowed to share common indicator values.

Although we demonstrated the use of HIAs on local ecosystem condition indicators, there is in fact no reason why it cannot be used with larger EAAs.
...
In our case, only two or three HIA levels had field data recorded in them, depending on the municipality, and the differences between HIAs in each municipality was relatively small.
This could mean that broader HIA definitions could have been used in our case.

There are other ways to achieve spatial gerenalisation of data whilst accounting for sampling biases.
Geostatistical models (e.g. krieging) arguable being the most common.
However, these approaches are more data demanding, making them less useful for local ECAs where data from outside the relatively small EAAs should not be allowed to influence the indicator values.
They are also more complicated and thus less transparent to the end users, therefore at a compromise with the ideal for simplicity (@czucz_selection_2021).
In addition, local area managers may get useful information from seing the indicators values uniquely for each HIA (@fig-forest) and in that way gain an understanding about where problems with ecosystem deterioration are greatest.
