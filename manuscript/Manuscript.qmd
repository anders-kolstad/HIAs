...
This could mean that the species or ecosystem function in question is completely lost fro the ecosystem.
...
The SEEA EA framework recommends that ecosystem condition indicators give an unbiased representation of the condition [table 1 in @czucz_selection_2021; see also @unitednations2021 §2.87], and that condition indicators are recorded uniquely for each ecosystem asset (EA) so that the condition in the ecosystem accounting area (EAA) can be inferred from an area weighted mean based on the relative sizes of the EAs [@unitednations2021 §5.54].
Ecosystem assets are defined as "ecological entities" (meaning areas) about which information is sought and about which statistics are ultimately compiled [@unitednations2021].
This recommendation for representativity in the indicators means that spatially biased data are ill suited, especially if sampling intensity varies along gradient of anthropogenic pressures and hence ecosystem condition.
This recommendation for the spatial resolution also means that when a condition metric is sampled coarsely, for example in an opportunistic and spatially biased field campaign, then it is very likely that you will not have a enough data points to to religably estimate an indicator value for each EA.
...
There are at least 3 ways to achieve this same complete areal coverage (i.e. spatialisation) that is relatively unbiased, from originally point-based indicator values:

a.  Predict (project) values from a statistical model that accounts for the effect of environmental variation on the variables
b.  Take a central tendency (e.g. the mean) from the sample population and project it to all EAs of a given ecosystem type.
c.  Take a central tendency from stratified sections of the sample polulation and projection them to the EAs, adhering to the corresponding strata.
...
Besides Nordre Follo, which do not contain any HIA-0 areas (i.e. relatovely wild areas), the three municipalities had areas in all HIA classes.
Yet, none of the three municipalities had survey data from all HIAs.
Therefore, when we transferred the mean and SD for each HIA (@fig-forest) over to the ecosystem delineation map (@fig-spread-example), some mire polygons in the ecosystem delineation map did not get assigned any data, and were therefore not included (i.e. did not have any influence on) in the following aggregation to ecosystem accounting areas (EAA).
...
In this paper we have demonstrated a GIS-based workflow for making use of spatially biased field data to produce representative ecosystem condition indicators at a local municipal scale.
This approach is valuable especially for local assessments were we do not want to data from outside the EAA inform or influence our estimates for condition indicators inside the EAA.
...
This implies that the stratification was warranted in the latter case (Trenching), since field data from urban or sub-urban mires could not be said to represent the status of mires in more remote wilderness areas.
In the case for Alien species, stratifying the data can be predicted to have less directional effect on average, but we did find a difference between HIAs 1 and 2 in Nord-Aurdal municipality, indicating that the impact from alien species in these two strata of varying infrastructure differs (@fig-forest).
...
Therefor, we see little risk in using the stratification process also on indicators where the relationship between the indicators and the HIA are weak, although we stress that not accounting for potentially other, more relevant predictors of indicator values, would leave the indicator as spatially biased as the original variable.

We have also demonstrated how Bayesian updating and resampling can be used instead of traditional arithmetic aggregation of indicators, from EAs to EAAs, to produce probability density functions for indicator values.
...
The variance is commonly used, usually calcualated from the variation between spatially distant sampling points.
However, the variance is not reliable for low sample sizes.
For example, even if the true numerical population has a large variance, it is not impossible to sample two or three numbers that are very close to each other, and this sample would then display a low variance which is not reflective of the true variance.
Knowing this, an often used approach is to set a sample size limit, and refrain from estimating indicator values unless there are more that a given number of data points.
In our test municipality Nordre Follo, we had only 1-3 data points for each HIA strata, yet we were able to use these data regardless because the Bayesian updating produces a reliable, and relatively broad, unceratinty range for the estimate.
...
The UN standard for ecosystem accounting has been tailed towards national accounts.
Similarly, reporting of Ecosystem Accounts to Eurostat, mandatory for all EU and EEA countries from 2026, is required for each major ecosystem type, but with to requirements for sub-national resolution.
...
Local ecosystem accounts istead rely more on remote sensing or field measurements.
Remote sensing holds great promise for delivering valuable information to ecosystem accounts across all scales, yet there are some ecosystem characteristics that can not easily be sensed remotes.
...
For example, Gran ad Nord-Aurdal have not recorded any field data from their most urban areas (HIA 3), and could make these areas a priority in upcoming field campaigns in order to get more complete spatial indicator coverage.
...
This kind of information can give grounds for a more regional assessment, for example looking at the effectys of policies on ecosystem condition trajectories.
One way to accomplish level of synthesis is to design and use indicators that are common across EAAs, or to compile and publish existing data in a standardised format.
In Norway, EIAs are frequently being produced at scales, and with information that is highly relevant for local ECAs, yet the data is not standardised or available as a common resource.
In contrast, the Biodiversity Net Gain program in England enforces developers to record and compile data on ecosystem extent and condition at very fine resolution in order to document they effect on nature and ensure a positive net gain from their activities.
The data is made publicly available in a national data base (?) and could be used to design indicators that are informative at a municipal scale, larger than the project level scale at which they were recorded.
The other major difference between Biodiversity Net Gain and the Norwegian EIA is the way the BNG and the work to estimate the biodiversity metric, is initiated with a wall-to-wall delineation of nature types, and relevnt ecosystem condition variables are then recorded explicitly in these, whereas
