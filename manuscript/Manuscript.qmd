...
# Set local to ensure Norwegian letters get imported correctly. I have this line in my Rprofile:
...
# and calculate area
# However, for this we want to use th 100m resolution data,
# so lets prepare that first

# Rather than transforming the entire 100m raster, 
# I mask it first, and then transform
muni3x <- muni3 |>
  st_transform(st_crs(infra_big))
#st_crs(muni3x) == st_crs(infra_big)
...
# Then I vectorise
infraMuni3 <- eaTools::ea_homogeneous_area(infraMuni3,
  groups = infrastructureIndex
)
# calculate area and get municipality variables
infraMuni3 <- infraMuni3 |>
...
muniPlot <- tm_shape(muni3) +
  tm_borders(lwd = 3, col = "black") +
  tm_facets(by = "Municipality", ncol = 3) +

  tm_shape(terrestrial) +
  tm_fill(col = "lightgreen", alpha = 1) +
  ...
  tm_shape(nature3) +
  tm_polygons(col = "red", border.col = "red", lwd = 3)+
...
(NA_overview <- tm_shape(muni3 |> filter(Municipality == "Nord-Aurdal")) +
  ...
  tm_add_legend(type = "fill",
                labels = c("Land", "Survey coverage", "Condition data"),
...
#tmap_save(tm = NA_overview,
#       "images/NA_overviewA.jpg",
#       width=4,
#       height=4)
...
#ggsave("images/forest-plot_100m.jpg",
#       plot=forest_plot,
#       width = 8,
#       height = 6)
```
...
```{r}
# The means are mostly within the 95%CI, withtwo exceptions.
# The bars are the predicted distributions of the domain data, and not 
# traditinal errors. 
# They should not be presented as uncertainty intervals, since the predicted point estimate can be outside the predicted range. 
...
```{r EEA-plotAreaHeights}
#| eval: true
#| cache: true
# Now the same as above, but where the height of the blue bars reflect the area and not the count
bin_width <- 0.05
breaks = seq(0, 1, by = bin_width)

combineAll_area <- combineAll |>
  unnest(cols = c(realSamples, realAreas)) |>
  mutate(
    sample_bin = cut(
      realSamples,
      breaks = breaks,
  ...
  select(Municipality, indicator2, realSamples, sample_bin, realAreas) |>
  group_by(Municipality, indicator2, sample_bin) |>
  summarise(area = sum(realAreas)) |>
  ungroup() |>
  group_by(Municipality, indicator2) |>
  mutate(area_norm = area / max(area, na.rm = TRUE)) |>
  ungroup()
...
temp <- combineAll |>
  filter(indicator == "i_ditch",
     Municipality == "Nord-Aurdal")
temp2 <- combineAll_area |>
  filter(Municipality == "Nord-Aurdal",
  indicator2 == "Trenching")
...
Its purpose is support policies, landuse planning and management decitions, and making it easier to account for nature by making the environmental costs of certain policies and practices visible to decision-makers.
...
An important requirement of SEEA EA is that condition metrics must give an unbiased spatial representation of the condition for a given area [table 1 in @czucz_selection_2021; see also @unitednations2024 §2.87].
...
While spatially and temporally representative estimation is essential, this requirement severely limits the types of usable data.
Local ECAs are particularly constrained by data availability, 
because ecosystem condition and environmental quality monitoring programmes are often designed to be representative at national scale. 
This can lead to _ad hoc_ and potentially unrepresentative selections of variables and indicators.
...
Wall-to-wall raw data, such as satellite imagery, is spatially representative through the property of being spatially exhaustive [@williams_overcoming_2024], although grain size also plays a role (cf. the ecological fallacy [@robinson2009]).
...
Nationally designed surveys will suffer from sparse sampling, and, depending on the reason for the survey, may also suffer from spatially biased sampling, thus leaving gaps in ecosystem asset coverage from a local management perspective.
...
Other options include imputing values to unsampled areas using some type of model or spatial generalisation [@barton2023].
...
via the related term *value transfer* [@unitednations2022; @ncaves; @grammatikopoulou2023; @barton2023].
...
knitr::include_graphics("../images/forest-plot_100m.jpg")
```
...
The sample sizes are also so small that they have little influence on the predicted indicator range which becomes determined mainly by the national data. 
That is also why the point estimate for HIA-3 in Gran is outside the predicted range, 
