...
In this paper we have demonstrated a generalisable workflow for making use of scarce and spatially biased field data to produce representative ecosystem condition indicators at a local municipal scale, combining direct estimation for point estimates with variance smoothing for better predictions of indicator uncertainty (@fig-workflowExample).
...
We will focus on two novel approaches in turn, describing their virtues and uses: post-stratification for minimising adverse effects of spatial and environmental bias in the data, and empirical Bayes variance smoothing for calculating realistic indicator uncertainties even for small sample sizes.
...
We think that direct estimation is a critical virtue for local point estimates (i.e. relying only on local data) and simply use the area weighted mean of the data that are available for any give strata.
...
This problem gets exaggerated when data is truncated, as indicators are (they are bound between zero and one), because one is more likely to sample a series of identical one's or zero's which would yield a variance of zero.
...
This is an advantage, especially for local ecosystem condition assessments where data scarcity is likely to be more of an issue than for national assessments.
...
As we would not want to presnet indicator values without ayn uncertainty estimates, then our approach make it possible to estimate indicator values for a much greater area than if we were forced to set a minimum sample size threshold.
With increasing sample sizes, local data get more influence on predicting the indicator distributions.
...
With a premise that local ECAs should use direct estimation and not be influence by data from outside the EAA, we have developed two generic approaches that allows the inclusion of sparse and spatially biased datasets.
ECAs are often limited by indicator availabilities, and making use of less-than-ideal datasets could greatly alleviate this problem.
...
As ECAs and the SEEA EA continues to develop and mature we find it reasonable that more emphasis is placed on the estimation and disclosure of indicator uncertainties, as well as on spatial extrapolation and aggregation, and our work is pointing in this direction.
