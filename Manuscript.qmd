...
<!-- Local ECAs, challenges there -->

<!-- Explain spatial bias -->
...
At its core is the idea that data on ecosystem condition can be summarised in the form of summary statistics for selected metrics (condition indicators and variables) presented in generalised tables that describe the condition of a larger ecosystem accounting area (EAA).

With intentional similarities to national accounts, which deal with the economic activity at the scale of nations, SEEA EA accounts have mainly been developed for and used at national scales [@czucz_no_2025].
...
Compared to accounts with large EAAs, local accounts face even greater challenges with resolution and availability of data.

Ecosystem condition accounts are expected to provide a single estimate (a point estimate) for any condition metric over the entire EAA, representing the the whole reporting period. 
Accordingly, condition metrics must give an unbiased spatial representation of the condition for a given area [table 1 in @czucz_selection_2021; see also @unitednations2024 §2.87].
...
<!-- There is a lot of environmental or bidiversity data which are collected oppurtunistically, with no real sampling design. This includes citizen science data, but also ...
Such data can and often will suffer from spatial (geographical and/or environmental biases), with sampling effors varying systamtically with some important variables, like distance to roads,  population density or general human activity -->
...
The sampling intensity of national monitoring programs is typically inadequate for fine-scale applications,
and depending on the purpose of the survey, may also suffer from spatially biased sampling, thus leaving gaps in ecosystem asset coverage from a local management perspective.
