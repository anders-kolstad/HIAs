...
Condition metrics under the SEEA EA are either classed as condition variables or as indicators [@unitednations2024].
...
Condition indicators on the other hand, are always on a dimensionless scale between 0 and 1, representing a gradient of deviation from a reference condition (indicator value of 1) towards a completely deterriorated ecosystem (indicator value 0) [@jakobsson2020].
The reference condition is what the current condition is compared against, for example an "intact ecosystem", but its definition can vay according to the ecosystem type or the aim of the account.
Reference levels are numerical representations of the reference condition, and these are most often derived via expert-based approaches, followed by data-driven aapproaches and reference sites [@nicholson_thomas_towards_2025]. 
...
These datasets will therefore typically not have enough data points for robust local inference from such data using design-based estimation methods, and one would need to explore different types of predictive modelling and approaches from within the field of small area statistics [@ghosh1994; @guldin2021; @fay_iii_estimates_1979; @breidenbach_small_2012].
...
This may raise concerns in some cases about the relevancy of the condition metric when its estimated value is purely synthetic and _out of sample_, meaning its value is not informed by any direct information for the given area [@vogt_spatial_2023].
As a consequence, local ECAs may not be able to leverage data from large nature monitoring programs, or if they wanted to, they would find scarce relevant guidelines for how to tackle this _out of sample_ problem.

<!-- spatial bias and data projection issues -->

The SEEA EA is designed as a spatial accounting framework, meaning condition metrics should ideally be recorded (or modeled) for all contiguous spaces (patches) of a specific ecosystem type (a common definition of an *ecosystem asset*) [@nicholson_thomas_towards_2025].
Furthermore, ECAs are expected to provide a single estimate (a point estimate) for its condition metrics that represents the entire EAA and the the whole reporting period, and give an unbiased spatial representation of the condition for a given area [table 1 in @czucz_selection_2021; see also @unitednations2024 §2.87].
For sampled (i.e. non-exhaustive) data, this requires some spatial projection of data to cover ecosystem assets where they are not recorded [@shannon_toward_2024].
...
<!-- SEA gives domain level or unit level estimates and errors. This is not completely agreeable with SEEA EA -->
...
<!-- vogt2023 : we could've used a spatial model to borrwo strength from neighbouring areas, and not all of Norway -->

<!-- We could have used existing models and tool (spatial EBLUP) to get estimates and errors, and then just ignored the estimates, or compared to our estimates. Could HIAs have been a random effect? -->

<!-- FH shrinkage depends on with-area sampingvariance, but the sample variance is unreligable when n is very small.
BFH (Battese–Harter–Fuller) is a unit level approach, and there the shrinkage depends on within-area n (and noise) -->

<!-- EBLUP empirical Best Linear Unbiased Predictor, assumes the variance componenys are unknown, and estimates themusing real data -->

<!-- SME is typically used to try and reduce the error around local estimates, but we often want to increase it, becuse very low n and truncated data give too low SDs -->

<!--  In area-level models, the auxiliary information is only available for the small area level. In unit-level models, the auxiliary information is available for all elements of the population in the small area. National statistics oftern use unit level approaches like FH, so why not ecosystem accounting. -->
