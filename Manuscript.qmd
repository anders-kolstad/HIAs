...
Its purpose is support policies, landuse planning and management decisions, 
and making it easier to account for nature by making the environmental costs of certain policies and practices visible to decision-makers.
The System of Environmental-Economic Accounting - Ecosystem Accounting (SEEA EA) is the most widely used framework for ecosystem accounting. 
...
Condition variables describe certain ecosystem characteristics deemed important for ecosystems in good condition, 
and they are typically in their original units of measurement, or as ratios. 
Condition indicators on the other hand, are on a dimensionless scale between 0 and 1, representing a gradient of deviation from 
a reference condition (indicator value of 1) towards a completely deterriorated ecosystem (indicator value 0).
The reference condition is what the current condition is compared against, 
and its definition can vay according to the ecosystem type or the aim of the account.
Typical reference conditions are defined as something like an "intact ecosystem".

<!-- Local ECAs, challenges  -->
...
However, there is a surge of new examples of regional, municipal and project level accounts, 
motivated at least partly by the need to match the scale of the information to the scale of decision-making processes [@simensen_naturregnskap_2024; @gorman_decision_2024].
Although data shortages are universal for ECAs at all scales, they may be even more prominent at local scales (ref).
As we shall see, there are important differences between a typical national scale ECA and a more local ECA in what type of data they have available 
to them and in how the available data can and should be used to design condition variables or indicators that are relevant 
(e.g. have relevant resolutions, accuracy and precision) and mathematically defensible (e.g. provide unbiased estimates).


One such difference is the use of point observations or sample data from field campaigns. 
Ecosystem condition and environmental quality monitoring programmes are often designed to be representative at national scales,
and there will typically not be enough data points for robust local inference from such data using design-based estimation methods, 
...
In small area statistics, the local estimates are informed by data from outside the local area, something referred to as _borrowing strength_. 
This may raise concerns in some cases about the relevancy of the condition metric when its estimated value is _out of sample_ (REF and def).
As a consequence, local ECAs may not be able to leverage data from large nature monitoring programs, or is they wanted to, 
they would find scarce relevant guidelines for how to tackle this _out of sample_ problem.      

<!-- spatial bias -->

Another difference between national and more small-scale ECAs related to the use of spatially biased, or preferrentially sampled data,
meaning data that are sampled without a known or reproducible sampling design, and in contrast to spatially representative data, which do have such a design.



Ecosystem condition accounts are expected to provide a single estimate (a point estimate) for any condition metric over the entire EAA, 
representing the the whole reporting period, 
...
The SEEA EA framework is spatially explicit, meaning these metrics should ideally be recorded (or modeled) for all contiguous spaces (patches) 
of a specific ecosystem type (a common definition of an *ecosystem asset*).
Wall-to-wall raw data, such as satellite imagery or remote sening data in general, is spatially representative through the property of being spatially exhaustive [@williams_overcoming_2024], 
and automatically gives data for all ecosystem assets.  
For local applications of remote sensing data it is worth noting that grain size also plays a role 
...
and still require field surveys.
These data are spatially incomplete, and to comply fully with the SEEA EA, they need to get projected on to ecosystem assets where they are not recorded (ref).
A subsequent area-weighted mean for the EAA will then produce the point estimate [@unitednations2024 §5.54].
...
or they may leverage statistical tools that aims to overcome the original sampling bias, such as geostatistics (ref).
...
There is, however, a lack of guidelines in the SEEA EA for how to create condition metrics specifically for local ECAs which tackle the issues of spatial bias and which address the _out of sample_ issue.   
...
Other options include imputing values to unsampled areas using more complex types of spatial models [@barton2023].
Spatial generalisation is not unique to ecosystem condition accounts, 
and it is much discussed in the ecosystem service accounting community of practice 
via the related term *value transfer* [@grammatikopoulou2023].
Advantages of value transfer include low cost by leveraging freely available seondary data. 
The major drawback of value transfer or generalisation is estimation error due to spatial and temporal biases.
...
Depending on the data that goes into them, models can make very accurate estimates, especially when the areas are large (e.g. regions or nations), but their reliability diminishes for smaller areas (e.g., municipalities) due to data scarcity and model uncertainty [@venter2024].
...
Ecosystem accounts should ideally reflect conditions only inside the accounting area (i.e. direct estimation) 
and are arguable more useful when reflecting local management and land use policies, unaffected by the actions of neighboring or ecologically similar administrative units.
...
<!-- Small area statistics and out of sample problems -->
...
However, a critical step in ecosystem accounting is upscaling — synthesising fine-grained data to characterise larger areas, effectively reducing grain size or increasing support [@bierkens2002; @kolstad_spatial_2025].
